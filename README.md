...
- `pointcloud_colors` (optional, live only): `[N, 3]` float32 RGB colors derived from the *live* force field (R=fx, G=fy, B=fz). Generated by `LiveTactileProcessor` so colors reflect any runtime `force_field_scale`.
- `pointcloud_forces` (optional, live only): `[N, 3]` float32 per-point force values (fx, fy, fz) — produced by `LiveTactileProcessor` for pointcloud visualization.
